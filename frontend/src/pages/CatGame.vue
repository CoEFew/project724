<template>
  <div class="min-h-screen relative overflow-x-hidden theme-modern">
    ...
    <div class="pointer-events-none absolute inset-0 -z-10" aria-hidden="true">
      <div class="absolute inset-0 bg-[radial-gradient(90%_70%_at_70%_100%,rgba(99,102,241,0.45),transparent_60%),radial-gradient(60%_60%_at_0%_0%,rgba(59,130,246,0.35),transparent_60%),linear-gradient(180deg,#0b1020,#0b1120)]"></div>
    ...
    <div v-if="isPortrait" class="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm grid place-items-center text-center p-6">
    ...
    <div class="w-full max-w-7xl mx-auto px-4 pt-8 pb-44"><!-- bottom padding for sticky controls -->
      ...
      <section class="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md p-4 flex flex-col items-center justify-center">
        ...
        <div class="w-full mt-3">
          ...
          <div class="hpbar"><div class="hpfill boss" :style="{ width: bossHpPct + '%' }"></div></div>
        </div>
          ...
          <div class="grid grid-cols-12 gap-3 items-center">
            ...
            <div class="col-span-12 md:col-span-5 lg:col-span-4">
              <div class="fighter-card relative" :class="playerBeingHit ? 'shake' : ''">
                ...
                <div class="hpbar"><div class="hpfill" :style="{ width: playerHpPct + '%' }"></div></div>
            ...
            <div class="col-span-12 md:col-span-7 lg:col-span-8 flex flex-wrap items-center justify-center md:justify-end gap-2">
              ...
              <button
                ...
                @click="discardAndDraw"
                :disabled="turnEnded || discardsLeft === 0 || selectedIndices.size === 0"
              >ทิ้ง & จั่ว ({{ selectedIndices.size }})</button>
    ...
    <div v-if="showDeck" class="fixed inset-0 z-[60] bg-black/70 backdrop-blur-sm grid place-items-center p-4">
      <div class="w-full max-w-4xl rounded-2xl border border-white/10 bg-white/10 p-4 max-h-[85vh] overflow-y-auto">
        <div class="flex items-center justify-between mb-3 sticky top-0 -m-4 p-3 pl-4 pr-4 bg-white/10 backdrop-blur-md border-b border-white/10 rounded-t-2xl z-10">
          ...
          <button class="px-3 py-1 rounded-lg border border-white/10 bg-white/10 text-slate-200 hover:bg-white/20" @click="showDeck=false">ปิด</button>
        </div>
    ...
    <div v-if="showHowTo" class="fixed inset-0 z-[70] bg-black/70 backdrop-blur-sm grid place-items-center p-4">
      <div class="w-full max-w-4xl rounded-2xl border border-white/10 bg-white/10 p-4 max-h-[85vh] overflow-y-auto">
        <div class="flex items-center justify-between mb-3 sticky top-0 -m-4 p-3 pl-4 pr-4 bg-white/10 backdrop-blur-md border-b border-white/10 rounded-t-2xl z-10">
          ...
          <button class="px-3 py-1 rounded-lg border border-white/10 bg-white/10 text-slate-200 hover:bg-white/20" @click="showHowTo=false">ปิด</button>
        </div>
    ...
    <div v-if="showScoreModal"
         ...
         role="dialog" aria-modal="true" aria-labelledby="scoreTitle">
...
.card.selected { transform: translateY(-6px) scale(1.2); z-index: 10; }
.card.mini { @apply aspect-[3/4] p-2 rounded-xl; }
.card-head { @apply flex items-center justify-between font-bold; font-size: 1.25rem; /* ~text-xl */ }
.card.mini .card-head { font-size: .9rem; /* bigger than text-xs */ }
.card-body { @apply flex items-center justify-center h-[calc(100%-1.9rem)]; font-size: 3.75rem; /* ~text-6xl */ }
.card.mini .card-body { font-size: 1.9rem; /* ~text-3xl */ }
